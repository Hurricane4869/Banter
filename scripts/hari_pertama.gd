...
func _input(event):
	if GameStarted.game_started == true:
		if event.is_action_pressed("quest"):
			...
			handphone.play_backwards("Start_Quest")
	if event.is_action_pressed("quest") or (event.is_pressed() and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT) and not tutorial_finished:
		click += 1
		print_debug(click)
		if $UI/Tutorial/FIrstTutorial.is_visible_in_tree():
			$UI/Tutorial/FIrstTutorial.hide()
			$UI/Tutorial/SecondTutorial.show()
			click += 1
			# print_debug(click)
		elif $UI/Tutorial/SecondTutorial.is_visible_in_tree() and click >= 3:
			$UI/Tutorial.hide()
			timer.start()
			handphone.play("Start_Quest")
			open_handphone_sound.play()
